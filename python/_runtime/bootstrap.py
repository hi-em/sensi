...
def bootstrap(layout_path: Path | None = None) -> Context:
    ...
    # Source layouts -- read-only. Resolves to <repo root>/randomized_layouts/
    # These files are never overwritten by the agent.
    # python/_runtime/bootstrap.py → _runtime/ → python/ → repo root is parents[2]
    repo_root = Path(__file__).resolve().parents[2]
    layout_input_dir  = repo_root / "randomized_layouts"  # source -- never overwritten
    layout_output_dir = repo_root / "resulting_layout"    # analysis results written here
    ...
    edited_layout_path = repo_root / "edited_layout.json"
