...
Run (from python/, UTF-8 on Windows, with the .venv that has the backend deps):
...
_PY = Path(__file__).resolve().parent          # <repo>/python
_TEAM = _PY.parent                             # <repo> (repo root)
_REPO = _TEAM                                  # .env lives at the repo root
