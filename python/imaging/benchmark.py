...
Run (from python/, UTF-8 on Windows):
    python -m imaging.benchmark
Outputs to docs/week08/benchmark/.
"""
...
_PY_DIR = Path(__file__).resolve().parents[1]          # <repo>/python
_TEAM_DIR = Path(__file__).resolve().parents[2]        # <repo> (repo root)
_REPO = _TEAM_DIR                                      # .env lives at the repo root
