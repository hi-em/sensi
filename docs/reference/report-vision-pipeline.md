...
[`python/imaging/client.py`](../../python/imaging/client.py) — `generate_image(prompt, reference_b64=None)`.
...
[`python/imaging/prompt.py`](../../python/imaging/prompt.py) — `build_room_prompt(room, scores, persona)`.
...
[`python/api/server.py`](../../python/api/server.py):
...
`web/src/report/`: `ReportScreen.jsx` (orchestrates `/api/report` + lazy
